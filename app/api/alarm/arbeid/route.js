...
export async function POST(request) {
  try {
    ...
    const { data: links } = await supabase.from("werkbon_links").select("nummer, klant, arbeid_gemeld").eq("klus_id", klus_id).limit(1);
    ...
    const { data: tr } = await supabase.from("tijdregels").select("minuten").eq("klus_id", klus_id);
    ...
    const { data: ka } = await supabase.from("kaart").select("id").eq("type", "klus").eq("klus_id", klus_id).limit(1);
    ...
    if (managers.length && kaartId) {
      await supabase.from("melding").insert(managers.map((m) => ({ ontvanger: m.code, kaart_id: kaartId, van: "Arbeid-alarm", soort: "activiteit", tekst })));
    }
    ...
    if (link) await supabase.from("werkbon_links").update({ arbeid_gemeld: true }).eq("klus_id", klus_id);
